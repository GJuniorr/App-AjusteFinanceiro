...
class projecaoP extends StatefulWidget {
  ...
  @override
  State<projecaoP> createState() => _projecaoPState();
}

class _projecaoPState extends State<projecaoP> {
  ...
  Future selectRenda() async {
       var resultadoSR = await conn.execute("Select * from projeçãop WHERE ID_Usuario = '$IDU' ");
      ...
      if (count > 0) {
        ...
        Map data = element.assoc();
        rendaID = int.tryParse(data['ID_ProjeçãoP']);
    ...
    var resultadoDadosG = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' ORDER BY Categoria DESC");
    List<Map<String, String>> listG = [];

    for (final row in resultadoDadosG.rows) {
      final dataG = {
        'ID_ProjeçãoP': row.colAt(0)!,
    ...
    var resultadoDadosADDRenda = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' AND Tipo = 'Renda+do mês'  ORDER BY Valor DESC"); 
    ...
    var resultadoDadosRendaMensal = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' AND Tipo = 'Renda Mensal'  ORDER BY Valor DESC");
    ...
    var resultadoDadosADDRendaM = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' AND Categoria = 'Renda'  ORDER BY Valor DESC");
    ...
    var resultadoDadosADDDespesa = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' AND Tipo = 'Despesa+do mês'  ORDER BY Valor DESC");
    ...
    var resultadoDadosDespesaM = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' AND Tipo = 'Despesa Mensal'  ORDER BY Valor DESC");
    ...
    var resultadoDadosADDDespesaM = await conn.execute("SELECT * FROM projeçãop WHERE ID_Usuario = '$IDU' AND Categoria = 'Despesa'  ORDER BY Valor DESC");
  ...
  Future removerItem(int rendaID)async {
      ...
      "DELETE FROM projeçãop WHERE ID_ProjeçãoP = :ID_ProjeçãoP ",
      {
        'ID_ProjeçãoP': rendaID,
      },
    );
                              ...
                              Row(
                                children: [
                                  Padding(
                                    ...
                                    onPressed: () {
                                    Navigator.push(context, 
                                    MaterialPageRoute(builder: (context) => Addrendaprojecaop(
                                      IDU: IDU
                                    )));
                                  }, icon: Icon(Icons.add,
                                  color: Colors.black,)),
